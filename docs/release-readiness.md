# SKS 8.1.2 Release Readiness

This document is the current fail-closed release contract for `sneakoscope`
8.1.2. The current package version on this branch is 8.1.2. It is a readiness
...
- the 7.6.0 to 8.1.2 upgrade smoke and focused 8.0.4-to-8.0.5 resolved-CLI
...
smoke report, and platform-gate reports under the 8.1.2 release evidence root.
...
README, changelog, built output, and release evidence must agree on 8.1.2.
...
Automation must stop before this approval. It must not claim that 8.1.2 is
...
```bash
npm view sneakoscope@8.1.2 version dist.integrity dist.tarball --json
npm view sneakoscope dist-tags --json
```

Then install `sneakoscope@8.1.2` into a fresh isolated prefix and rerun the
installed-package smoke. Completion requires the registry version to be
8.1.2, `latest` to resolve to 8.1.2, integrity to match, and the fresh install
to pass.
