# SKS 9.2.1 Release Readiness

## Current decision

**SOURCE TAG CONDITIONAL / NPM PUBLICATION OPERATOR-OWNED.**

9.2.1 makes auto official-models routing follow the operator: registering and enabling codex-lb IS choosing the gateway (selected models keep running through it until the provider is un-registered, when a ChatGPT-OAuth host converges back onto its own identity at the next bridge start or sync), OpenRouter picks stay sticky per model route, and explicit pins beat registration both ways. It also makes the official-models pin durable: two settings writers (defaultDesktopBridgeServiceSettings and the catalog-sync serializer) dropped the official_passthrough field, so an explicit gateway/passthrough choice silently reverted to auto on the next rebuild or sync. Found by post-publish verification of 9.2.0 on this machine; both writers now carry the field, with a serializer round-trip regression test. Regenerate the isolated 7.6.0 to 9.2.1 upgrade smoke, the full release gate DAG, pack receipt, and release-check stamp from the clean candidate commit.
...
npm view sneakoscope@9.2.1 version --json --registry https://registry.npmjs.org/
