...
This is **required**. SKS discovers which chat and user to pair by reading that
`/start` message. It only accepts a `/start` sent in a **private** chat. Without it,
setup fails with:
...
```bash
printf '%s' "123456789:ABC…" | sks telegram setup --bot-token-stdin --project-root "$PWD" --json
```
