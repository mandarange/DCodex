...
The current implementation handoff is bounded to typecheck, one clean build, focused changed-surface tests, `release:check:affected`, one `release:check:confidence`, and `npm publish --dry-run --json --registry https://registry.npmjs.org/ --tag latest --access public`. That sequence exercises the non-publishing package path but does not create publish authorization. Publication is a distinct repository-maintainer workflow: the manual main-only OIDC job runs `release:check:full`, verifies the source-bound release stamp, and only then reaches its intentional publish step. The removed `publish:npm` and `release:publish` aliases are historical only and cannot serve as 7.3.0 evidence.
