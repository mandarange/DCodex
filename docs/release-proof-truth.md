# Release Proof Truth — 8.3.1

## Current assertion

8.3.1 is **SOURCE TAG CONDITIONAL / NPM PUBLICATION OPERATOR-OWNED**. The
...
All release artifacts bound to 8.3.0 or an earlier commit are historical. They
must not be renamed, copied, or treated as 8.3.1 evidence.
...
| All checked version authorities report 8.3.1 | passed-hermetic in the integrated working tree; not exact-commit proof | `release:version-truth` checked 15 package/lock/plugin, TypeScript version and re-export, Rust manifest/lock/metadata, dist manifest, README, changelog, and release-metadata-script surfaces with zero warnings |
| The reported 8.3.1 package is ready to publish | not proved | requires a clean exact-commit build, release gates, package receipt, provenance, and the operator's final registry checks |
...
lifecycle, or require a live Paseo session as 8.3.1 release proof. The owned
...
Before any release claim, regenerate and verify current 8.3.1-bound artifacts
