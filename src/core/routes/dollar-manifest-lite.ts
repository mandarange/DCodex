...
const NARUTO_DESCRIPTION = '$Naruto is the lightweight Codex official subagent workflow and the default for non-trivial execution. The Sol Max parent dynamically scales useful independent children from a bounded four/six/eight starting policy up to the safe capacity ceiling (default 12); max_threads is a cap, never a target. Healthy host CPU/memory/API budget may raise the automatic count toward that ceiling, and live pressure may shrink it. Capacity is the minimum of ready DAG width, disjoint ownership, verifier/tool capacity, reserved thread slots, and positive marginal usefulness. Child routing is fixed to Luna Max mechanical, Sol High implementation, Sol Max judgment, and Terra Medium long-context/Computer Use/Browser/ImageGen execution. Duplicate slices and overlapping writes are blocked; explicit --agents remains authoritative.';
...
const DOLLAR_COMMANDS_LITE_BASE = [
  ...
  { command: '$Work', route: 'execution alias to the Codex official subagent workflow', description: NARUTO_DESCRIPTION },
  ...
  { command: '$Super-Search', route: 'provider-independent source intelligence', description: 'Run Super-Search source acquisition, source normalization, claim/proof ledgers, and provider-independent citation evidence without requiring provider-specific credentials.' },
