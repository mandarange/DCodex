...
const DOLLAR_COMMANDS_LITE_BASE = [
  ...
  { command: '$Cleanup', route: 'destructive TriWiki blank-state cleanup', description: 'Explicitly plan and permanently remove active TriWiki memory, indexes, caches, reports, and managed AGENTS projections without retaining a previous generation.' },
  { command: '$Align', route: 'code-only TriWiki alignment', description: 'Create or replace TriWiki by rereading current repository code only and publishing an exhaustive file/symbol/line and source-relation navigation index without using prior memory.' },
...
const DOLLAR_COMMAND_ALIASES_LITE_BASE = [
  ...
  { canonical: '$Cleanup', app_skill: '$cleanup' },
