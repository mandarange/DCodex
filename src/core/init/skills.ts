...
async function installOfficialSkills(root: any) {
  ...
  const skills = {
    ...
    'fast-mode': `---\nname: fast-mode\ndescription: Dollar-command route for $Fast-Mode, $Fast-On, and $Fast-Off global Codex Desktop Fast mode toggles.\n---\n\nUse when the user invokes $Fast-Mode, $Fast-On, $Fast-Off, or asks to turn SKS Fast mode on/off. Prefer \`sks fast-mode on|off|status|clear --json\`. By default on/off updates the global Codex Desktop config so GPT-5.6 Fast (official service_tier) persists and also keeps .sneakoscope/state/fast-mode.json in sync for SKS workers. Use \`--project\` only when the user explicitly wants project-local worker preference without touching global Codex config. Explicit runtime flags still win: \`--fast\`, \`--no-fast\`, and \`--service-tier standard|fast\` override the saved preference for that run. Finish with a short status and Honest Mode; do not start Naruto or broad implementation for a toggle-only request.\n`,
    'fast-on': `---\nname: fast-on\ndescription: Alias for $Fast-On global Codex Desktop GPT-5.6 Fast enablement.\n---\n\nUse the same rules as fast-mode. Run or instruct \`sks fast-mode on --json\`, then report Global (desktop), Project (sks workers), state file, and the fact that explicit per-run flags still override the saved preference.\n`,
    ...
    'align': `---\nname: align\ndescription: Dollar-command route for $Align code-only TriWiki rebuild including wiki/pack.\n---\n\nUse when the user invokes $Align or asks to rebuild TriWiki from current repository code. Align is independent of triwiki-cleanup / $Cleanup and does not require a cleanup receipt. Run \`sks align prepare\` then \`sks align run\` (or \`sks align run\` directly). context-graph.json is exhaustive authority; context-pack and managed AGENTS.md are bounded projections. Fail closed on caps/binary/oversized sources. Do not nest subagents. Agents never run \`sks doctor --fix\`. Finish reflection and Honest Mode once at the end.\n`,
