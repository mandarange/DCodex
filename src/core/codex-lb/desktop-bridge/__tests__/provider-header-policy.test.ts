...
import { startDesktopBridge, stopDesktopBridge, type DesktopBridgeConfig, type DesktopBridgeHandle } from '../index.js';
...
const inbound = {
  ...
  'x-sks-session-id': 'thread-1', 'http-referer': 'https://client.example', 'x-title': 'Desktop',
};
...
test('Codex-LB and OpenRouter policies strip OAuth/cookies and inject only the selected credential', async () => {
  ...
  for (const headers of [lb, openrouter]) {
    assert.equal(headers.cookie, undefined); assert.equal(headers['x-sks-session-id'], undefined); assert.equal(headers['x-remove-me'], undefined);
...
test('one bridge process keeps concurrent provider credentials and model rewrites request-local', async () => {
  ...
  const config: DesktopBridgeConfig = {
    ...
    listenHost: '127.0.0.1', listenPort: bridgePort, allowedPathPrefixes: ['/v1/'], allowedOrigins: ['app://codex'], connectTimeoutMs: 1_000, idleTimeoutMs: 5_000,
  };
  try {
