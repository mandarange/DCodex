...
- Make image generation work under a selected codex-lb provider. Four defects sat between a working proxy and the route. The provider closes `response.completed` with an empty `output` and delivers the image on the preceding `response.output_item.done` (`item.result`) or `response.image_generation_call.partial_image` (`partial_image_b64`); SKS read only the completed envelope and its fallback matcher could match neither shape, so both paths reported `missing_b64_image_output`. `src/core/responses-stream.ts` now folds streamed items back into the envelope, for json_schema structured output as well as images. imagegen also re-implemented `process.env[key] || env-file` key resolution instead of using the fingerprint-bound `loadCodexLbEnv`, so a stale exported `CODEX_LB_API_KEY` shadowed the SKS-managed file and the proxy answered 401 while capability still reported ready. The codex-lb path was gated behind an opt-in flag that switched off the only surface answering when codex-lb is selected — provider selection now enables it, an explicit `0`/false still disables it — and the request model comes from the served catalog, because `config.toml`'s `model` may be a slug the key cannot use (403 `model_not_allowed`).
